...
def show_jump():
    global  ui, ui_df, pos_df, combo_choices, combo_choices1, combo_choices2, combo_choices3, combo_box, combo_box1, combo_box2, combo_box3, combo_box, combo_box1, combo_box2, combo_box3, print_text
    
    days = int(print_text.get())
    ...
    tree = ttk.Treeview(root1, height=20)
...
def show_jump_down():
    ...
    cols = list(combine.columns)
    tree = ttk.Treeview(root2, height=20)
    tree.grid(column=0,row=0, sticky = 'NW')
    tree["columns"] = cols
...
describe_label = tk.Label(mainframe, text='往回幾天計算' )
describe_label.grid(column=2, row=0,  sticky=(N), padx=1, pady=1, ipadx=20)
