...
def show_jump_down():
    ...
    cols = list(combine.columns)
    tree = ttk.Treeview(root2, height=20, show='headings')
    tree.grid(column=0,row=0, sticky = 'NW')
    tree["columns"] = cols
...
def show_info():
    global  ui, ui_df, pos_df, quota_table

    try:
        if quota_table == None:
            quota_table = pd.read_csv('../example.csv')
    except:
        pass
    
    row_select = ui.getSelectedRow()
    print(ui.getSelectedRow())
    comp_name = ui_df.loc[row_select, '證券名稱']
    quota_table_select = quota_table.query('證券名稱 == @comp_name')
    quota_table_select = quota_table_select.reset_index(drop=True)

    root3= tk.Tk()
    root3.wm_title(f'限額資料表')
    ...
    tree = ttk.Treeview(root3, height=20, show='headings')
    tree.grid(column=0,row=0, sticky = 'NW')
    tree["columns"] = cols
    ...
    child_id = tree.get_children()[-20]
    tree.focus_set(child_id)
